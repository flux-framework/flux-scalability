...
#
# A bunch of environment variables should be set a upper level script. 
# However, for a case where some variables are missing, 
# this script picks default values to these variables. 
#
...
SCAL_TST_RESULTS_DIR=${SHARNESS_TEST_SRCDIR}/test-results-data/sched-scalability
...
case $SCAL_TST_SCHED_POLICY in
    fcfs|FCFS)
    FLUX_SCHED_OPTIONS="sched-params=queue-depth=16 plugin=sched.fcfs"
    ;;
    easy|EASY)
    FLUX_SCHED_OPTIONS="sched-params=queue-depth=16 plugin=sched.backfill plugin-opts=reserve-depth=1"
    ;;
    conservative|CONSERVATIVE)
    FLUX_SCHED_OPTIONS="plugin=sched.backfill plugin-opts=reserve-depth=-1"
    ;;
    hybrid100|HYBRID100)
    FLUX_SCHED_OPTIONS="plugin=sched.backfill plugin-opts=reserve-depth=100"
    ;;
    wreckrun|WRECKRUN)
